...
- **New panels expose pre-existing inconsistencies in adjacent surfaces.** Surfaced during 12.1.7 S3 visual review: shipping the Financial Pulse strip with a "Revenue YTD" card put the existing top-stat "Revenue" (different metric, same word) on visual collision course. Same review surfaced the "Active tab containing PENDING-labeled projects" contradiction that pre-existed but wasn't visible until newer chrome forced a side-by-side comparison. This is healthy — adjacent-surface drift is invisible until something forces the eye to compare. Stage 13 should plan a deliberate naming/labeling sweep across CommandCenter, Dispatch, ProjectDetail (when it exists) before pilot. Treat these as expected outcomes of polish-as-we-go, not regressions.
- **Sample-based denominator predictions can undercount.** During 12.1.7 S3 scope-prep, a `SELECT * FROM crew_utilization LIMIT 5` returned 5 rows — extrapolated to a "0/4 deployed" expected value. The full count was 7 (4 PMs + 3 field_crew). The code rendered the right number because the denominator is computed dynamically, but the prediction was wrong. Lesson: when verifying expected values pre-build, use full COUNTs, not sample-of-N.
...
- **Crews Deployed** — `crew_utilization` view filtered to `role IN ('pm','field_crew')`. Numerator = rows with `active_load > 0`; denominator = total rows. Test data: 0/7 deployed · 0% (4 PMs + 3 field_crew = 7 deployable, all with `active_load = 0`; sub-line "no active assignments"). The earlier 0/4 prediction was based on a sample-of-5 SQL output and undercounted; the live full count is 7.
...
- Crews: 7 rows with role∈(pm,field_crew) (4 PMs + 3 field_crew), all `active_load = 0` → 0/7. Sub-line correctly resolves to "no active assignments" because numerator === 0. ✓ (Pre-build prediction was 0/4 based on a sample-of-5 SQL output; live full count is 7. No code change needed — denominator is computed dynamically from query results.)
...
**Post-ship visual review findings (2026-05-03 later, after the 9a9a9d2 commit):** Two pre-Session-3 inconsistencies became visible once the Financial Pulse strip rendered alongside existing CommandCenter chrome. Both pre-existed but weren't visually salient until adjacent panels forced a side-by-side comparison. Captured as Stage 13 carry-forwards in Tech Debt and as a Lessons Banked entry ("New panels expose pre-existing inconsistencies in adjacent surfaces"):
1. **"Revenue" label collision** — top stat strip "Revenue" ($6,900 = sum of contract fees on visible projects, pipeline value) vs Financial Pulse "Revenue YTD" ($12,450 = sum of paid invoice amounts year-to-date). Both valid, both labeled with "Revenue". Reconcile by renaming the top stat (`Pipeline Value` / `Booked Revenue`) or aligning both to one definition.
2. **Active tab contains PENDING-labeled projects** — internally consistent (tab = operational state, label = lifecycle state) but reads as a contradiction. Resolve via a status-display map (`pending → Scheduled`, etc.) — coordinate with the existing Tech Debt to convert `projects.status` to a Postgres enum, same surface area.

**Crews Deployed denominator clarification:** SQL after the smoke test confirmed 8 total users in `crew_utilization` (1 owner + 4 PMs + 3 field_crew). With the role filter `IN ('pm','field_crew')` the denominator is 7, not the 4 predicted from the LIMIT-5 sample during scope prep. Rendered value `0/7 · 0%` is correct. No code change — denominator is computed dynamically from the live query.
