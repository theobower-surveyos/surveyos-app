...
export default function CommandCenter({ profile, projects, teamMembers, onProjectSelect, onCreateProject, onArchiveProject }) {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100%', animation: 'fadeIn 0.5s ease-out' }}>
      ...
      <DeploymentModal isOpen={isDeploying} onClose={() => setIsDeploying(false)} teamMembers={teamMembers} onDispatch={async (data) => { const newProject = await onCreateProject(data); showToast('Dispatch initialized — node deployed to map'); if (newProject?.id) navigate(`/dispatch/${newProject.id}`); }} />
